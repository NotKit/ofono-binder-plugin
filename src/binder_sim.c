    ...
    RADIO_AIDL_INTERFACE interface_aidl)
{
    ...
    if (interface_aidl == RADIO_AIDL_INTERFACE_NONE) {
        ...
        if (parcel_size >= sizeof(guint32) * 3) {
            ...
            DBG("parcel_size=%d,sw1=0x%02X,sw2=0x%02X,%s", parcel_size,
                resp->sw1, resp->sw2, hex ? hex : "(null)");
    ...
    void* data)
{
    ...
    if (self->interface_aidl == RADIO_AIDL_INTERFACE_NONE) {
        ...
        // FIXME: figure out how to calculate it properly
        gsize parcelable_size = sizeof(gint32) * 9
            + G_ALIGN4((strlen(hex_path) + 1) * 2)
            + G_ALIGN4((strlen(hex_data) + 1) * 2)
            + G_ALIGN4((strlen(empty) + 1) * 2)
            + G_ALIGN4((strlen(aid) + 1) * 2);

        /* Non-null parceable */
        gbinder_writer_append_int32(&writer, 1);
        /* Parceable inner size + size of this integer */
        gbinder_writer_append_int32(&writer, parcelable_size + sizeof(gint32));
    ...
    case OFONO_SIM_PASSWORD_PHNET_PIN:
        if (!enable) {
            code = self->interface_aidl == RADIO_SIM_INTERFACE ?
                RADIO_SIM_REQ_SUPPLY_SIM_DEPERSONALIZATION :

    ...
    BinderSlot* slot)
{
    ...
    RADIO_AIDL_INTERFACE data_interface = modem_interface != RADIO_AIDL_INTERFACE_NONE ?
        RADIO_DATA_INTERFACE : RADIO_AIDL_INTERFACE_NONE;
    RADIO_AIDL_INTERFACE messaging_interface = modem_interface != RADIO_AIDL_INTERFACE_NONE ?
        RADIO_MESSAGING_INTERFACE : RADIO_AIDL_INTERFACE_NONE;
    RADIO_AIDL_INTERFACE network_interface = modem_interface != RADIO_AIDL_INTERFACE_NONE ?
    ...
    void *user_data)
{
    ...
    if (cap) {
        ...
        slot->caps = binder_radio_caps_new(plugin->caps_manager, slot->name,
            slot->client[modem_interface], slot->watch, slot->data, slot->radio, slot->sim_card,
    ...
    BinderSlot* slot)
{
    ...
    slot->sim_card = binder_sim_card_new(slot->client[sim_interface], slot->config.slot);
    slot->sim_card_state_event_id =
    ...
    slot->network = binder_network_new(slot->path, slot->client[network_interface],
        slot->client[data_interface], slot->client[modem_interface],
        slot->name, slot->radio, slot->sim_card, slot->sim_settings,
        &slot->config);

    GASSERT(!slot->data);
    slot->data = binder_data_new(plugin->data_manager, slot->client[data_interface],
        slot->client[network_interface],
    ...
    if (binder_plugin_multisim(plugin) &&
        ...
        slot->caps_check_req = binder_radio_caps_check(slot->client[modem_interface],
    ...
    BinderPlugin* plugin)
{
    if (plugin->flags & BINDER_PLUGIN_HAVE_CONFIG_SERVICE) {
        if (!plugin->radio_config) {
            plugin->radio_config = radio_config_new_with_version_and_interface_type
                (RADIO_CONFIG_INTERFACE_1_1, plugin->settings.interface_type);
...
binder_plugin_connect_to_interface(
    ...
    slot->instance[aidl_interface] = radio_instance_new_with_modem_slot_version_and_interface(
        dev, slot->name, slot->path, slot->config.slot, slot->version,
        aidl_interface);
    slot->client[aidl_interface] = radio_client_new(slot->instance[aidl_interface]);
    ...
    if (!binder_plugin_is_slot_client_connected(slot) && need_client) {
        ...
        if (radio_client_connected(slot->client[modem_interface])) {
            ...
            slot->client_event_id[CLIENT_EVENT_CONNECTED] =
                radio_client_add_connected_handler(slot->client[modem_interface],
    ...
    BinderPlugin* plugin)
{
    ...
    if (ps->interface_type == RADIO_INTERFACE_TYPE_HIDL) {
        ...
        plugin->radio_config_watch_id =
        gbinder_servicemanager_add_registration_handler(plugin->svcmgr,
            RADIO_CONFIG_AIDL_FQNAME, binder_plugin_service_registration_proc, plugin);
    }

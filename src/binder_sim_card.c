    ...
    GBinderReader* reader)
{
    gint32 card_state = 0;
    gint32 pin_state = 0;
    gint32 gsm_umts_index, cdma_index, ims_index = 0;
    guint32 num_apps;
    ...
    DBG("parcel_size=%d, card_state=%d, universal_pin_state=%d, gsm_umts_index=%d, "
        "ims_index=%d, cdma_index=%d, num_apps=%d",
        parcel_size, card_state, pin_state, gsm_umts_index, cdma_index,
        ims_index, num_apps);
    ...
    BinderSimCardObject* self)
{
    if (self->status_req) {
        ...
        DBG("binder_sim_card_get_status code=%d", code);
